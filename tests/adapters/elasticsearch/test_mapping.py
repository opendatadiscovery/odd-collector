...
from odd_collector.adapters.elasticsearch.mappers.stream import DataEntity, DataEntityType, map_data_stream, map_data_stream_template
from odd_models.models import DataSetField, DataSetFieldType, Type


class TestDataStreamMapping(TestCase):
    def setUp(self) -> None:
        self.oddrn_generator = ElasticSearchGenerator(
            host_settings="test"
        )
        config = ElasticsearchPlugin(host="localhost", port="9200", type="elasticsearch", name="test_elastic")
    ...
    def test_stream_mapping(self):
        ...
        data_entity = map_data_stream(data_stream, tempalte_metadata, None, self.oddrn_generator)

        self.assertIsNotNone(data_entity)

        self.assertEqual(data_entity.oddrn, '//elasticsearch/host/test/indexes/mystream')
    ...
    def test_map_data_stream_template(self):
        ...
        data_stream_entity = map_data_stream(data_stream, tempalte_metadata, None, self.oddrn_generator)

        data_stream_oddrn = [data_stream_entity.oddrn]
        data_entity = map_data_stream_template(tempalte_metadata, data_stream_oddrn, self.oddrn_generator)
        ...
        self.assertListEqual(data_entity.data_entity_group.entities_list, data_stream_oddrn)
