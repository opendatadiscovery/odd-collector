...
def map_table(oddrn_generator: RedshiftGenerator, mtables: MetadataTables, mcolumns: MetadataColumns, database: str) -> List[DataEntity]:
    data_entities: List[DataEntity] = []
    column_index: int = 0

    for mtable in mtables.items:
        data_entity_type = TABLE_TYPES_SQL_TO_ODD.get(mtable.base.table_type, DataEntityType.UNKNOWN)
        oddrn_path = "views" if data_entity_type == DataEntityType.VIEW else "tables"

        oddrn_generator.set_oddrn_paths(**{'schemas': mtable.schema_name, oddrn_path: mtable.table_name})
        ...
        if mtable.all.table_creation_time is not None:
            data_entity.updated_at = mtable.all.table_creation_time.replace(tzinfo=pytz.utc).isoformat()
            data_entity.created_at = mtable.all.table_creation_time.replace(tzinfo=pytz.utc).isoformat()
        ...
        if data_entity.type == DataEntityType.VIEW:
            data_entity.data_transformer = extract_transformer_data(mtable.all.view_ddl, oddrn_generator)
