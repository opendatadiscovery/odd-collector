...
def map_db_service(db_service_name: str, group_oddrns: list[str], oddrn_path_name: str,
                   oddrn_generator: MssqlGenerator) -> DataEntity:
    return DataEntity(type=DataEntityType.DATABASE_SERVICE,
...
def extract_schemas_entities_from_tables(tables_entities: list[DataEntity], oddrn_generator: MssqlGenerator) \
        -> list[DataEntity]:
    schemas_list: set[str] = set([extract_table_schema_name(table_entity) for table_entity in tables_entities])
    ...
    for schema_name in schemas_list:
        ...
        schemas_tables.append(map_db_service(schema_name, tables_oddrns, "schemas", oddrn_generator))
