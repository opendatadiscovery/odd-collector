...
def map_table(
    ...
    tables: List[tuple],
    columns: List[tuple],
    integration_engines: List[tuple],
    database: str,
) -> List[DataEntity]:
    ...
    for table in tables:
        ...
        data_entity_type = (
            DataEntityType.VIEW
            if mtable.engine in ["View", "MaterializedView"]
        ...
        if (
            "metadata_modification_time" in mtable
            and mtable.metadata_modification_time is not None
        ):
            data_entity.updated_at = mtable.metadata_modification_time.replace(
                tzinfo=pytz.utc
            ).isoformat()

        # Dataset
        data_entity.dataset = DataSet(
            rows_number=int(mtable.total_rows)
        ...
        if data_entity.type == DataEntityType.VIEW:
            data_entity.data_transformer = extract_transformer_data(
                mtable, oddrn_generator, integration_engines
            )
