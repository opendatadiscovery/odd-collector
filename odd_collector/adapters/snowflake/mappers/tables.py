...
from . import MetadataNamedtuple, _data_set_metadata_schema_url, _data_set_metadata_excluded_keys, ColumnMetadataNamedtuple
...
def map_table(oddrn_generator: SnowflakeGenerator, tables: list[tuple], columns: list[tuple]) -> list[DataEntity]:
    ...
    for table in tables:
        ...
        oddrn_generator.set_oddrn_paths(**{'schemas': metadata.table_schema, oddrn_path: metadata.table_name})
        ...
        data_entity.dataset = DataSet(
            rows_number=int(metadata.row_count) if metadata.row_count is not None else None,
            field_list=[]
        )
        ...
        while column_index < len(columns):
            ...
            if column_metadata.table_schema == metadata.table_schema and column_metadata.table_name == metadata.table_name:
