...
class DatasetFieldMapper:
    def map_feature(self, raw_feature_data: Feature, oddrn_generator: FeastGenerator) -> List[DataSetField]:
        return self.__extract_dataset_fields(
            name=raw_feature_data.name,
            logical_type=raw_feature_data.dtype,
            description=', '.join(": ".join(label_pair) for label_pair in raw_feature_data.labels.items()),
            oddrn_generator=oddrn_generator
        )

    def map_entity(self, raw_entity_data: Entity, oddrn_generator: FeastGenerator) -> List[DataSetField]:
        return self.__extract_dataset_fields(
    ...
    def __extract_dataset_fields(self, name: str, logical_type: ValueType, description: str, oddrn_generator: FeastGenerator) -> List[DataSetField]:
        result = []

        odd_type = self.__map_feature_type(logical_type=logical_type)
        parent_oddrn = oddrn_generator.get_oddrn_by_path("features", name)
        result.append(self.__map_dataset_field(
            oddrn=parent_oddrn,
            name=name,
        ...
        child_logical_type = TYPES_FEAST_FEATURE_TO_ODD.get(logical_type.value, {}).get('child', None)
    ...
    def __map_dataset_field(
            self, oddrn: str, name: str, logical_type: ValueType, odd_type: str, description: str = None, parent_oddrn: str = None
    ) -> DataSetField:
        return DataSetField(
            oddrn=oddrn,
    ...
    def __map_feature_type(self, logical_type: ValueType):
        return TYPES_FEAST_FEATURE_TO_ODD.get(logical_type.value, {}).get('field_type', Type.TYPE_UNKNOWN)
