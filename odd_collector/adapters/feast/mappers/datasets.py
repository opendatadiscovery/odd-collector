...
class FeatureViewMapper:
    def __init__(
            self, raw_feature_view_data: FeatureView, raw_entities_data_dict: Dict[str, Entity],
    ...
    def get_feature_view(self) -> DataEntity:
        self.__oddrn_generator.set_oddrn_paths(featureviews=self.__raw_feature_view_data.name)
        features = [
        ...
        return DataEntity(
            ...
            metadata=[metadata_extractor.extract_dataset_metadata(
                MessageToDict(self.__raw_feature_view_data.to_proto())
            )],
            dataset=DataSet(
                parent_oddrn=None,
                rows_number=0,
                field_list=list(flatten(features))
            )
        )
