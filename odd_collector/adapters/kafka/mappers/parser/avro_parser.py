...
from .nodes import Inherited, UnionNode, ObjectNode, ArrayNode, MapNode, EnumNode, AvroPrimitiveNode, Node
...
class AvroParser(AbstractParser):
    ...
    def map_schema(self,
                   schema: RawSchema,
                   references: List[RawSchema] = None) -> List[Field]:
        ...
        for field in fields:
            ...
            field['parent_field_oddrn'] = f"{self.base_oddrn}/{field['parent_field_oddrn']}" \
    ...
    def __parse_schema(self,
        ...
        if isinstance(node_schema, str):
            ...
            if node_schema in self.__cached_records:
                if node_schema in AvroPrimitiveNode.get_primitives():
                    log.warning('Node definition found both in cached records and primitives. '
        ...
        if isinstance(node_type, List):
            ...
            union_node.inner_nodes = [
                self.__parse_schema(node_schema=s,
                                    parent_node=union_node,
                                    inherited=Inherited(is_nullable=True))
                for s in node_type if s != 'null'
            ]

            return union_node

        if node_type == 'record':
            name = f"{node_schema['namespace']}.{node_schema['name']}" \
            ...
            object_node.properties = [node for node in [
                self.__parse_schema(node_schema=s | s['type'] if isinstance(s['type'], dict) else s,
        ...
        if node_type == 'array':
            ...
            array_node.items = self.__parse_schema(node_schema=node_schema.get('items'), parent_node=array_node)
        ...
        if node_type == 'map':
            ...
            map_value_node = self.__parse_schema(node_schema=node_schema['values'], parent_node=map_node)
            map_value_node.is_value = True

            map_node.key = AvroPrimitiveNode(type='string', is_key=True, parent_node=map_node)
