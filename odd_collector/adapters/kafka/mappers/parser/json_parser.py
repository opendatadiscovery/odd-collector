...
class JsonParser(AbstractParser):
    ...
    def map_schema(self,
        ...
        # parsing definitions
        for def_name, def_schema in (schema.get('definitions') or schema.get('$defs', {})).items():
            self.__definitions[def_name] = self.__parse_schema(
                node_schema=def_schema,
                inherited=Inherited(name=def_name)
            )

        # parsing external definitions
        for r in references:
            for def_name, def_schema in (r.get('definitions') or r.get('$defs', {})).items():
                self.__definitions[def_name] = self.__parse_schema(
                    node_schema=def_schema,
                    inherited=Inherited(name=def_name)
                )
        ...
        for field in fields:
            ...
            field['parentFieldOddrn'] = f"{self.base_oddrn}/{field['parentFieldOddrn']}" \
    ...
    def __parse_schema(self,
                       ...
                       inherited: Inherited = Inherited()) -> Node:
        ...
        if node_type in JsonPrimitiveNode.get_primitives():
            ...
            return JsonPrimitiveNode(
                ...
                enum_types=[
                    self.__parse_schema(node_schema=s)
                    for s in node_schema.get('enum')
                ] if 'enum' in node_schema else None,
                description=description,
                default=default,
                parent_node=parent_node,
                is_nullable=inherited.is_nullable
