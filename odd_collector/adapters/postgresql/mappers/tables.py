...
def map_table(oddrn_generator: PostgresqlGenerator, tables: List[tuple], columns: List[tuple], database: str) -> List[DataEntity]:
    ...
    for table in tables:
        ...
        data_entity.dataset = DataSet(
            rows_number=int(metadata.table_rows) if metadata.table_rows is not None else None,
            field_list=[]
        )
