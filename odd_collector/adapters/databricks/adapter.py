...
class Adapter(AbstractAdapter):
    ...
    async def get_schemas_per_catalog_list(self, catalogs: list) -> list[tuple]:
        schemas = [
            (catalog, schema)
    ...
    async def get_tables_metadata(self, schemas_per_catalog: list) -> list[dict]:
        tables = [
    ...
    @staticmethod
    def group_metadata(data: list) -> dict:
    ...
    async def get_data_entity_list(self) -> DataEntityList:
        ...
        schemas_per_catalog = await self.get_schemas_per_catalog_list(catalogs)
        metadata_raw = await self.get_tables_metadata(schemas_per_catalog)
        metadata = self.group_metadata(metadata_raw)
        ...
        for catalog_name in metadata:
            ...
            catalog_entities.append(
                map_catalog(self.__oddrn_generator, catalog_name, schema_entities_tmp)
            )
