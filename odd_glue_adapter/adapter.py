...
class GlueAdapter:
    ...
    def get_transformers_runs(self, transformer: DataEntity = None) -> Iterable[DataEntity]:
        if transformer is None:
            return flatten([self.get_transformers_runs(t) for t in self.get_transformers()])
    ...
    def __process_table_raw_data(self, raw_table_data: Dict[str, Any]) -> DataEntity:
        column_names = [c['Name'] for c in raw_table_data['StorageDescriptor']['Columns']]

        column_stats = flatten([
            map_column_stats(self.__get_stats_for_columns(cns, raw_table_data)['ColumnStatisticsList'])
            for cns in chunked(column_names, SDK_DATASET_COL_STATS_MAX_RESULTS)
        ])

        return map_glue_table(raw_table_data,
                              {col[0]: col[1] for col in column_stats},
                              self._oddrn_generator)

    def __get_stats_for_columns(self,
                                column_names: Iterable[str],
                                raw_table_data: Dict[str, Any]) -> Dict[str, Any]:
